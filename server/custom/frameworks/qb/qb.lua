...
local postCooldowns = {}
local webhook = "https://discord.com/api/webhooks/1316553219752857640/2dgXw0dDNXnwIM64qAd6Mm7QQxzmDAWKKudDMvaPlhz6sJHe_jB_Kzc3fCdfpAYAKZR5"
AddEventHandler("lb-phone:pages:newPost", function(post)
    local src = GetSourceFromNumber(post.number)
    local Player = QB.Functions.GetPlayer(src)
    if not Player then return end

    local cid = Player.PlayerData.citizenid

    if postCooldowns[cid] then
        return
    end
    postCooldowns[cid] = true

    local title = "📒 " .. cid .. " - " .. post.title
    local embed = {
        {
            ["color"] = 16753920,
            ["title"] = title,
            ["fields"] = {
                ...
                {
                    ["name"] = "💰 Giá tiền",
                    ["value"] = post.price and "$" .. FormatNumber(post.price) or "Liên hệ để biết giá",
                    ["inline"] = true
                },
                {
                    ["name"] = "📱 Số điện thoại",
            ...
            ["footer"] = {
                ["text"] = "F17City - Thành phố mộng mơ",
            }
        }
    }
    
    if post.attachment ~= nil then
        embed[1]["image"] = { url = post.attachment }
    end
    
    PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({ username = "Quản lý Trang vàng", embeds = embed }), { ["Content-Type"] = "application/json" })
end)