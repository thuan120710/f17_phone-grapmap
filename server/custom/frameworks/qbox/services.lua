...
RegisterCallback("qbx:services:getEmployees", function(source)
    ...
    local members = exports.dnc_core:GetGroupMembers(job.name, "job")

    for i = 1, #members do
        local member = members[i]
        local gradeLevel = member.grade
        local citizenid = member.citizenid
        local player = exports.dnc_core:GetPlayerByCitizenId(citizenid) or exports.dnc_core:GetOfflinePlayer(citizenid)
...
RegisterCallback("qbx:services:hireEmployee", function(source, playerId)
    ...
    local success = exports.dnc_core:AddPlayerToJob(target.PlayerData.citizenid, player.PlayerData.job.name, 0)

    if not success then
        return false
    end

    success = exports.dnc_core:SetPlayerPrimaryJob(target.PlayerData.citizenid, player.PlayerData.job.name)
...
RegisterCallback("qbx:services:fireEmployee", function(source, citizenid)
    ...
    local success = exports.dnc_core:RemovePlayerFromJob(citizenid, player.PlayerData.job.name)
...
RegisterCallback("qbx:services:setGrade", function(source, citizenid, newGrade)
    ...
    local success = exports.dnc_core:AddPlayerToJob(citizenid, player.PlayerData.job.name, newGrade)
...
RegisterCallback("qbx:services:removeMoney", function(source, amount)
    ...
    -- Rút tiền từ society: nhận vào tiền khóa
