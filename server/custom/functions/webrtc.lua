...
local function RevokeCredentialsCloudflare(source)
    ...
    PerformHttpRequest("" .. WEBRTC.TokenID .. "/credentials/" .. username .. "/revoke", function(status, body, headers, errorData)
...
local function GetCredentialsCloudflare(source)
    ...
    PerformHttpRequest("" .. WEBRTC.TokenID .. "/credentials/generate-ice-servers", function(status, body, headers, errorData)
        if status ~= 201 then
