...
BaseCallback("garage:valetVehicle", function(source, phoneNumber, plate, coords, heading)
    ...
    if Config.Valet.Price and not RemoveMoney(source, Config.Valet.Price, "Dịch vụ đỗ xe", "priority") then
        SendNotification(phoneNumber, {
            app = "Garage",
            title = L("BACKEND.GARAGE.VALET"),
            content = L("BACKEND.GARAGE.NO_MONEY"),
    ...
    if not vehicle then
        -- Hoàn tiền valet (đã trừ priority nên hoàn vào tienkhoa)
        AddMoney(source, Config.Valet.Price, "tienkhoa")
