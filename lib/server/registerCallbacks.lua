...
---@field rateLimitNotification? boolean # Enable notification when rate limit is reached
---@field rateLimitMessage? string # Custom message for rate limit notification
---@field rateLimitApp? string # App name for rate limit notification (default: Twitter)
...
local function CanPlayerTriggerCallback(source, callbackName, options)
    ...
    if options.rateLimit then
        ...
        if callsPastMinute >= options.rateLimit then
            ...
            -- Send notification if rateLimitNotification is set
            if options.rateLimitNotification then
                local phoneNumber = GetEquippedPhoneNumber(source)
                if phoneNumber then
                    local message = options.rateLimitMessage or "Bạn đã đạt giới hạn. Vui lòng quay lại sau 1 phút"
                    local app = options.rateLimitApp or "Twitter"
