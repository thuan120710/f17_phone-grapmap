...
-- ========================================
-- TWITTER COMMENT SYSTEM ADDITIONS
-- ========================================

-- Add tweet_id column to phone_twitter_messages if it doesn't exist
SET @col_exists = 0;
SELECT COUNT(*) INTO @col_exists
FROM INFORMATION_SCHEMA.COLUMNS
WHERE TABLE_NAME = 'phone_twitter_messages'
  AND COLUMN_NAME = 'tweet_id'
  AND TABLE_SCHEMA = DATABASE();

SET @sql = IF(@col_exists = 0,
    'ALTER TABLE phone_twitter_messages ADD COLUMN tweet_id VARCHAR(255) NULL',
    'SELECT "tweet_id column already exists" as message');

PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Add index for tweet_id if it doesn't exist
SET @idx_exists = 0;
...
SET @sql = IF(@idx_exists = 0,
    'ALTER TABLE phone_twitter_messages ADD INDEX idx_tweet_id (tweet_id)',
    'SELECT "idx_tweet_id index already exists" as message');

PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Add is_comment column to phone_twitter_messages if it doesn't exist
SET @col_exists = 0;
SELECT COUNT(*) INTO @col_exists
FROM INFORMATION_SCHEMA.COLUMNS
WHERE TABLE_NAME = 'phone_twitter_messages'
  AND COLUMN_NAME = 'is_comment'
  AND TABLE_SCHEMA = DATABASE();

SET @sql = IF(@col_exists = 0,
    'ALTER TABLE phone_twitter_messages ADD COLUMN is_comment BOOLEAN DEFAULT FALSE',
    'SELECT "is_comment column already exists" as message');

PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Add index for is_comment if it doesn't exist
SET @idx_exists = 0;
SELECT COUNT(*) INTO @idx_exists
FROM INFORMATION_SCHEMA.STATISTICS
WHERE TABLE_NAME = 'phone_twitter_messages'
  AND INDEX_NAME = 'idx_is_comment'
  AND TABLE_SCHEMA = DATABASE();

SET @sql = IF(@idx_exists = 0,
    'ALTER TABLE phone_twitter_messages ADD INDEX idx_is_comment (is_comment)',
    'SELECT "idx_is_comment index already exists" as message');
...
-- ========================================
-- TWITTER COMMENT SYSTEM VERIFICATION
-- ========================================

-- Verify the columns were added successfully
SELECT 'TWITTER COMMENT SYSTEM SETUP' as info;
SELECT
    COLUMN_NAME,
    DATA_TYPE,
    IS_NULLABLE,
...
WHERE
    TABLE_NAME = 'phone_twitter_messages'
    AND COLUMN_NAME IN ('tweet_id', 'is_comment')
    AND TABLE_SCHEMA = DATABASE()
...
WHERE
    TABLE_NAME = 'phone_twitter_messages'
    AND INDEX_NAME IN ('idx_tweet_id', 'idx_is_comment')
    AND TABLE_SCHEMA = DATABASE()
ORDER BY INDEX_NAME;

-- Sample data check
SELECT 'SAMPLE COMMENTS VERIFICATION' as info;
SELECT
    COUNT(*) as total_messages,
    SUM(CASE WHEN is_comment = 1 THEN 1 ELSE 0 END) as comment_messages,
    SUM(CASE WHEN tweet_id IS NOT NULL THEN 1 ELSE 0 END) as messages_with_tweet_id
FROM phone_twitter_messages;

-- Final confirmation
SELECT 'TWITTER COMMENT SYSTEM SETUP COMPLETE' as final_status;
SELECT 'tweet_id and is_comment columns added to phone_twitter_messages' as setup_info;
SELECT 'Indexes created for optimal performance' as setup_info;
SELECT 'System ready for Twitter comment functionality' as setup_info;
