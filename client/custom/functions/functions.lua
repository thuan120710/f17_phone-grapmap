...
function HasAccessToApp(appIdentifier, jobName, jobGrade)
    ...
    if blacklistedJobs then
        -- If blacklistedJobs is true, blacklist for everyone
        if type(blacklistedJobs) == "boolean" and blacklistedJobs then
            debugprint("App is blacklisted for everyone", appIdentifier)
            return false
        elseif table.type(blacklistedJobs) == "array" then
